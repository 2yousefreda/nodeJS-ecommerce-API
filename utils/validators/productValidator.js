...
exports.createProductValidator = [
  check("title")
    ...
    .isLength({ max: 100 })
    .withMessage("Product name must be at most 32 characters")
    .custom((val, { req }) => {
  ...
  check("price")
    ...
    .isNumeric({ min: 0 })
    .withMessage("Product price must be at least 0"),
  ...
  check("subcategories")
      ...
      SubCategory.find({ _id: { $exists: true, $in: subcategoryIds } }).then(
        (result) => {
          if (result.length < 1 || result.length !== subcategoryIds.length) {
            return Promise.reject(
              new Error(`No subcategories found with id: ${subcategoryIds}`)
            );
          }
        }
      )
    ...
    .custom((val, { req }) =>
      SubCategory.find({ category: req.body.category }).then((subcategory) => {
        ...
        const checker = subcategoryIds.every((subcatId) =>
          subcategoryIdsInDB.includes(subcatId)
        );
        if (!checker) {
          return Promise.reject(
            new Error(
              `Subcategory with id: ${req.body.subcategories} does not belong to category with id: ${req.body.category}`
            )
          );
        }
      })
  ...
  check("ratingsQuantity")
    .optional()
    .isNumeric({ min: 0 })
    .withMessage("Ratings quantity must be at least 0"),
